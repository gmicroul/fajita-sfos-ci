...
if [ ! -d "$KERNEL_DIR" ]; then
 ...
 echo ""
 echo "使用方法："
 echo " bash clean-kernel-drivers-v2.sh <kernel-directory>"
 echo ""
 echo "示例："
 echo " bash clean-kernel-drivers-v2.sh /home/runner/work/android/kernel/oneplus/sdm845"
 exit 1
fi
...
echo "2. 保留时钟驱动（不删除 mdss-dsi-pll-10nm、mdss-pll 等）"
echo "   注意：这些驱动对于启动来说是必需的"
echo ""
...
# 清理 drivers/Makefile 中对 esoc 的引用
echo " - 清理 drivers/Makefile 中对 esoc 的引用"
...
echo "9. 保留电源管理驱动（不删除 qpnp-fg-gen3、qpnp-smb2、smb-lib）"
echo "   注意：这些驱动对于启动来说是必需的"
echo ""
